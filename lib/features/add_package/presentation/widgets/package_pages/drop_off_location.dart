import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:gap/gap.dart';
import 'package:google_maps_flutter/google_maps_flutter.dart';
import '../../../../../core/helpers/redius_helper.dart';
import '../../../../../core/languages/languages.dart';
import '../../../../../core/languages/local_keys.g.dart';
import '../../../../../core/shared/validations.dart';
import '../../../../../core/themes/text_theme.dart';
import '../../../../../core/widgets/custom_text_field.dart';
import '../../../../../core/widgets/expanded_drop_down.dart';
import '../../../data/models/governments_model.dart';
import '../../cubits/cubit/add_package_cubit.dart';
import '../custom_package_column.dart';

class DropOffLocationInfo extends StatelessWidget {
  const DropOffLocationInfo({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<AddPackageCubit, AddPackageState>(
      buildWhen:
          (previous, current) =>
              previous.status != current.status ||
              previous.dropoffLocation != current.dropoffLocation ||
              previous.dropOffGovernorate != current.dropOffGovernorate,
      builder: (context, state) {
        final cubit = AddPackageCubit.get(context);
        final dropOffCity =
            state.dropOffCity.cityEn == ''
                ? null
                : Languages.currentLanguage.isArabic
                ? state.dropOffCity.cityAr
                : state.dropOffCity.cityEn;
        final dropOffGov =
            state.dropOffGovernorate.governmentEn == ''
                ? null
                : Languages.currentLanguage.isArabic
                ? state.dropOffGovernorate.governmentAr
                : state.dropOffGovernorate.governmentEn;
        return Form(
          key: cubit.dropoffInfoFormKey,
          child: CustomPackageColumn(
            children: [
              Gap(0),
              Center(
                child: Text(
                  LocaleKeys.dropoffLocation,
                  style: AppTextTheme.text18W600grey100,
                ),
              ),
              ClipRRect(
                borderRadius: BorderRadius.circular(RadiusHelper.kRadius8),
                child: AspectRatio(
                  aspectRatio: 1.7,
                  child: GoogleMap(
                    initialCameraPosition: const CameraPosition(
                      target: LatLng(30.04652587682479, 31.226533111935872),
                      zoom: 10,
                    ),
                    markers: {
                      Marker(
                        markerId: MarkerId('dropoff'),
                        infoWindow: InfoWindow(
                          title: LocaleKeys.dropoffLocation,
                        ),
                        position: LatLng(
                          state.dropoffLocation?.latitude ?? 0.0,
                          state.dropoffLocation?.longitude ?? 0.0,
                        ),
                      ),
                    },
                    onTap: (coords) {
                      cubit.setDropoffLocation(coords);
                    },
                    onMapCreated: (controller) {
                      cubit.setDropoffController(controller);
                    },
                  ),
                ),
              ),
              Gap(0),
              ExpandedDropdown(
                hint: LocaleKeys.government,
                items: state.governmentsModel.data,
                itemLabelBuilder:
                    (item) =>
                        Languages.currentLanguage.isArabic
                            ? item.governmentAr
                            : item.governmentEn,
                onChanged: (value) {
                  cubit.setDropOffGov(value ?? GovernmentsData.initial());
                },
                selectedValue: dropOffGov,
              ),
              Gap(0),
              ExpandedDropdown(
                hint:
                    state.dropOffGovernorate.governmentEn == ''
                        ? LocaleKeys.pleaseSelectGovernmentFirst
                        : LocaleKeys.city,
                items: state.dropOffGovernorate.cities,
                itemLabelBuilder:
                    (item) =>
                        Languages.currentLanguage.isArabic
                            ? item.cityAr
                            : item.cityEn,
                onChanged: (value) {
                  cubit.setDropOffCity(value ?? Cities.initial());
                },
                selectedValue: dropOffCity,
              ),
              Gap(0),
              CustomTextField(
                hint: LocaleKeys.deliveryAddress,
                controller: cubit.dropOffAddressController,
                validate: (value) {
                  return Validations.validateEmpty(value);
                },
              ),
            ],
          ),
        );
      },
    );
  }
}
